...
# -*- coding: utf-8 -*-
"""
rmsf_symmetry_correlation.py

Compute per-run RMSF correlations between homologous chain pairs (e.g., C vs G, D vs H, A vs E, B vs F)
across wild-type and mutant systems. Outputs Pearson and Spearman correlation coefficients,
with symmetry-breaking deltas. Excludes the mutation site from correlation calculations.
"""
...
def calc_correlation(x1, y1, x2, y2, exclude_resid=None):
    if x1 is None or x2 is None:
        return np.nan, np.nan, 0
    common = np.intersect1d(x1, x2)
    if exclude_resid is not None:
        common = common[common != exclude_resid]
    if len(common) == 0:
        return np.nan, np.nan, 0
...
def calc_for_single_run(run, mutant_key, info, res_maps):
    ...
    # WT homologous pair correlation
    r_xwt_p, r_xwt_s, n = calc_correlation(x_wt_m, y_wt_m, x_wt_r, y_wt_r, exclude_resid=resid)
    # Mutant homologous pair correlation
    r_xmt_p, r_xmt_s, _ = calc_correlation(x_mt_m, y_mt_m, x_mt_r, y_mt_r, exclude_resid=resid)
...
def main():
    print("=" * 80)
    print("RMSF Symmetry Correlation Analysis (Per-Run)")
    print(f"Runs: {RUNS}")
    print("=" * 80)
    ...
    # Column name mapping for descriptive output
    name_map = {
    ...
    for mutant_key, info in MUTANTS.items():
        ...
        print(f"    WT Symmetry:        {summary['Cross_WT_Pearson']:.4f} ± {summary['Cross_WT_SEM_P']:.4f}")
        print(f"    Mutant Symmetry:    {summary['Cross_Mut_Pearson']:.4f} ± {summary['Cross_Mut_SEM_P']:.4f}")
        print(f"    Symmetry Breaking:  {summary['Delta_Pearson']:.4f} ± {summary['Delta_SEM_P']:.4f}")
        print()
        ...
        # Add per-run values for Cross_WT and Cross_Mut (not Delta)
        for run in RUNS:
    ...
    if all_results:
        ...
        out_csv = BASE_DIR / "RMSF_symmetry_correlation.csv"
