...
# -*- coding: utf-8 -*-
"""
Heavy-atom contact counts between H2A(C) and H2B(D): WT vs mutants
Run: cd /media/lenovo/MyDrive/rm_wat && python contacts_CD.py
Time window: 100–500 ns; only runs 1/2/3 used; short runs discarded
"""
...
# ================= Configuration =================
H2A_SEL = 'resid 176:280 and not name H*'
H2B_SEL = 'resid 281:376 and not name H*'
CUT = 4.5
FRAME_STEP = 50
T_START_NS = 100.0  # start at 100 ns
T_END_NS = 500.0    # end at 500 ns
MIN_FRAMES = 350    # drop run if effective frames < this
RUN_NAMES = ['run1', 'run2', 'run3']
OUT_DIR = 'contacts_out'  # output directory
...
# ================= Single run: exact 100–500 ns =================
def compute_one_run(top, traj):
    u = mda.Universe(top, traj)
    h2a = u.select_atoms(H2A_SEL)
    h2b = u.select_atoms(H2B_SEL)
    if len(h2a) == 0 or len(h2b) == 0:
        raise ValueError(f'Selection empty: H2A={len(h2a)} H2B={len(h2b)}')
    ...
    for ts in u.trajectory[::FRAME_STEP]:
        t_ns = ts.time / 1000.0
        t = t_ns - 6.0  # shift time (original time - 6 ns)
        if t < T_START_NS:
            continue
        if t > T_END_NS:
            break
        dm = mda.lib.distances.distance_array(h2a.positions, h2b.positions)
        times.append(t)
...
def main():
    ...
    plt.tight_layout()
    out_png = os.path.join(OUT_DIR, 'contacts_CD.png')
