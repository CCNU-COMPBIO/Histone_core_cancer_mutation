...
def read_cpptraj_rmsd(path: Path) -> pd.DataFrame:
    """Read cpptraj RMSD output file with columns: #Frame RMSD."""
    df = pd.read_csv(
        path,
        comment="#",
...
def align_and_stack(series_list):
    """
    Align multiple (x, y) sequences to a common x-axis (intersection),
    return common x and Y matrix [n_runs, n_points].
    """
...
def plot_comparison(ax, systems, runs, base_path, display_names, filename):
    """
    Plot RMSD comparison between two systems on given axes.
    Returns True if both systems have valid data, False otherwise.
    """
    ...
    for system in systems:
        ...
        for run in runs:
            ...
            try:
                ...
                if df.empty:
                    print(f"Warning: {f} has no data after truncation, skipping.")
                    continue
                x = df["Frame"].to_numpy()
                y = df["RMSD"].to_numpy()
                per_run.append((x, y))
        ...
        if not per_run:
            print(f"Warning: No valid data for system {system} in file {filename}, skipping.")
            return False
        ...
        if PLOT_STD_BAND and len(per_run) > 1:
            ax.fill_between(
                plot_x,
                mean - std,
                mean + std,
